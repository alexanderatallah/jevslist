...
export const listInput=z.object({name:z.string().trim().min(3,"Give your list a name of at least 3 characters.").max(80),description:z.string().trim().min(12,"Add a clearer description, at least 12 characters long.").max(1000),creatorHandle:z.string().trim().transform(s=>s.replace(/^@/,"")).pipe(z.string().regex(/^[A-Za-z0-9_]{0,15}$/, "An X handle uses up to 15 letters, numbers, or underscores.")).optional().default("")});
